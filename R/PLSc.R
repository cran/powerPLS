...
PLSc <- function(X, Y, A, scaling = "auto-scaling", post.transformation = TRUE, eps = 0.01,
                 Y.prob = FALSE, transformation = "ilr") {
  ...
  return(list(X_loading = X_loading, Y_loading = Y_loading, X = X, Y = Y, B = B, M = M,
              T_score = T_score, Y_fitted = Y_fitted))
}
